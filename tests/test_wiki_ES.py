...
def basket_ball_wiki_es(wiki_client, init_indices):
    ...
    filepath = os.path.join(
        os.path.dirname(__file__), "fixtures", "wiki", "basket_ball_wiki_es.json"
    )
...
def test_undefined_WIKI_ES(wiki_es_undefined):
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        for _ in range(10):
            client.get(url="http://localhost/v1/places/osm:way:7777777?lang=fr")
...
def test_no_lang_WIKI_ES():
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        client.get(url="http://localhost/v1/places/osm:way:7777777?lang=ru")
