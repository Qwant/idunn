...
class WeatherClient:
    ...
    def fetch_weather_places(self, coord) -> list:
        ...
        url_weather_api = self.weather_url.format(
            lat=coord.get("lat"), lon=coord.get("lon"), appid=self.weather_key
        )

        weather_town = self.session.get(
            url_weather_api + "&units=metric", timeout=self.request_timeout
        )
