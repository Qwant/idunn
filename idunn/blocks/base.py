    ...
    def __init__(self, *args, **kwargs):
        if not args:
            if not self.BLOCK_TYPE:
                raise Exception("Missing BLOCK_TYPE in class %s", self.__class__.__name__)
...
class TypedBlockValidator:
    ...
    def validate(self, value, definitions=None, allow_coerce=False):
        ...
        return block_class.validate(value=value, definitions=definitions, allow_coerce=allow_coerce)
