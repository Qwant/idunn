...
* `/v1/events?bbox={bbox}&outing_category=<outing_category>,*&size={size}' ` to get a list of all events matching the given bbox and outing_category
    * `bbox`: left,bot,right,top e.g. `bbox=2.0,48.0,3.0,49.0`
    * `outing_category`: one value is accepted (e.g. `outing_category=concert | show | exhibition | sport | entertainment`)
